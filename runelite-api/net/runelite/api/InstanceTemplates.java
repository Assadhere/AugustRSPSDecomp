...
public enum InstanceTemplates {
   ...
   public static InstanceTemplates findMatch(int chunkData) {
      ...
      int y = (chunkData >> 3 & 2047) * 8;
      int x = (chunkData >> 14 & 1023) * 8;
      int plane = chunkData >> 24 & 3;
      InstanceTemplates[] var5 = values();
      int var6 = var5.length;

      for(int var7 = 0; var7 < var6; ++var7) {
         InstanceTemplates template = var5[var7];
         if (plane == template.getPlane() && x >= template.getBaseX() && x < template.getBaseX() + template.getWidth() && y >= template.getBaseY() && y < template.getBaseY() + template.getHeight()) {
            return template;
         }
      }
