...
public class ChatMessageManager {
   private static final Set<Integer> TUTORIAL_ISLAND_REGIONS = ImmutableSet.of(12336, 12335, 12592, 12080, 12079, 12436, new Integer[0]);
   ...
   public void onConfigChanged(ConfigChanged event) {
      if (event.getGroup().equals("textrecolor")) {
         ...
         var10000.invokeLater(var10001::refreshChat);
      }
   ...
   void colorChatMessage() {
      ...
      switch (chatMessageType) {
         ...
         case PUBLICCHAT:
         case MODCHAT:
            prefix = Text.removeTags(username).replace(' ', ' ');
            if (this.client.getLocalPlayer().getName().equals(prefix)) {
            ...
            } else if (this.client.isFriended(prefix, true)) {
         ...
         case CLAN_GUEST_MESSAGE:
            ...
            channelColor = isChatboxTransparent ? this.chatColorConfig.transparentClanChannelGuestName() : this.chatColorConfig.opaqueClanGuestChatChannelName();
      }
      ...
      Iterator var17 = chatColors.iterator();

      while(var17.hasNext()) {
         ChatColor chatColor = (ChatColor)var17.next();
   ...
   public void onScriptCallbackEvent(ScriptCallbackEvent scriptCallbackEvent) {
      ...
      boolean isChatboxTransparent = this.client.isResized() && this.client.getVarbitValue(4608) == 1;
      Color usernameColor = isChatboxTransparent ? this.chatColorConfig.transparentPrivateUsernames() : this.chatColorConfig.opaquePrivateUsernames();
      if (usernameColor != null) {
         Object[] objectStack = this.client.getObjectStack();
   ...
   private void loadColors() {
      ...
      ChatMessageType[] var1 = ChatMessageType.values();
      int var2 = var1.length;

      for(int var3 = 0; var3 < var2; ++var3) {
         ChatMessageType chatMessageType = var1[var3];
         Color defaultTransparent = getDefaultColor(chatMessageType, true);
         if (defaultTransparent != null) {
            this.cacheColor(new ChatColor(ChatColorType.NORMAL, defaultTransparent, true, true, getSettingsColor(chatMessageType, true)), chatMessageType);
         }

         Color defaultOpaque = getDefaultColor(chatMessageType, false);
         if (defaultOpaque != null) {
            this.cacheColor(new ChatColor(ChatColorType.NORMAL, defaultOpaque, false, true, getSettingsColor(chatMessageType, false)), chatMessageType);
         }
      }
   ...
   private void cacheColor(ChatColor chatColor, ChatMessageType... types) {
      ChatMessageType[] var3 = types;
      int var4 = types.length;

      for(int var5 = 0; var5 < var4; ++var5) {
         ChatMessageType chatMessageType = var3[var5];
   ...
   public void process() {
      QueuedMessage msg;
      while((msg = (QueuedMessage)this.queuedMessages.poll()) != null) {
         this.add(msg);
      }
   ...
   private void add(QueuedMessage message) {
      ...
      if (player == null || !TUTORIAL_ISLAND_REGIONS.contains(player.getWorldLocation().getRegionID())) {
         MessageNode line = this.client.addChatMessage(message.getType(), (String)MoreObjects.firstNonNull(message.getName(), ""), (String)MoreObjects.firstNonNull(message.getRuneLiteFormattedMessage(), message.getValue()), message.getSender());
   ...
   String formatRuneLiteMessage(String runeLiteFormatMessage, ChatMessageType type, boolean pmbox) {
      ...
      Iterator var7 = chatColors.iterator();
      ...
      while(true) {
         ...
         do {
            ...
            chatColor = (ChatColor)var7.next();
         } while(chatColor.isTransparent() != transparent);
