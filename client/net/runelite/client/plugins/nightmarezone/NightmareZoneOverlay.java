...
class NightmareZoneOverlay extends OverlayPanel {
   ...
   public Dimension render(Graphics2D graphics) {
      if (this.plugin.isInNightmareZone() && this.config.moveOverlay()) {
         ...
         this.panelComponent.getChildren().add(LineComponent.builder().left("Points: ").right(QuantityFormatter.formatNumber((long)currentPoints)).build());
         this.panelComponent.getChildren().add(LineComponent.builder().left("Points/Hr: ").right(QuantityFormatter.formatNumber((long)this.plugin.getPointsPerHour())).build());
         this.panelComponent.getChildren().add(LineComponent.builder().left("Total: ").right(QuantityFormatter.formatNumber((long)totalPoints)).build());
         return super.render(graphics);
      } else {
         if (this.absorptionCounter != null) {
            this.removeAbsorptionCounter();
            Widget nmzWidget = this.client.getWidget(13238272);
            if (nmzWidget != null) {
