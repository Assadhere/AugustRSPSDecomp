...
public class NightmareZonePlugin extends Plugin {
   ...
   private void checkOverload() {
      if (Instant.now().isAfter(this.lastOverload.plus(OVERLOAD_DURATION).minus(Duration.ofSeconds((long)this.config.overloadEarlyWarningSeconds())))) {
   ...
   private int calculatePointsPerHour() {
      ...
      Duration timeSinceStart = Duration.between(this.nmzSessionStartTime, now);
      return !timeSinceStart.isZero() ? (int)((double)currentPoints * (double)HOUR.toMillis() / (double)timeSinceStart.toMillis()) : 0;
   }
