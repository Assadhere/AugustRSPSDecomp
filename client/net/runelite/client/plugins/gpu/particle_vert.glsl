...
layout(location = 2) in ivec4 iTex;     // x=texId, y=size, z=rotation, w=spriteMeta

uniform mat4 worldProj;
uniform float brightness;
uniform vec3 camPos;    // Camera position for spherical billboarding
...
vec3 hslToRgb(vec3 hsl, float bright) {
  float hue = hsl.x / 64.0 + 0.0078125;
  float sat = hsl.y / 8.0 + 0.0625;
  float lum = hsl.z;

  float var11 = lum / 128.0;
  float r = var11, g = var11, b = var11;

  float var19 = (var11 < 0.5) ? var11 * (1.0 + sat) : var11 + sat - var11 * sat;
  float var21 = 2.0 * var11 - var19;

  float var23 = hue + 0.333333;
  if (var23 > 1.0) var23 -= 1.0;
  float var27 = hue - 0.333333;
  if (var27 < 0.0) var27 += 1.0;

  if (6.0 * var23 < 1.0) r = var21 + (var19 - var21) * 6.0 * var23;
  else if (2.0 * var23 < 1.0) r = var19;
  else if (3.0 * var23 < 2.0) r = var21 + (var19 - var21) * (0.666666 - var23) * 6.0;
  else r = var21;

  if (6.0 * hue < 1.0) g = var21 + (var19 - var21) * 6.0 * hue;
  else if (2.0 * hue < 1.0) g = var19;
  else if (3.0 * hue < 2.0) g = var21 + (var19 - var21) * (0.666666 - hue) * 6.0;
  else g = var21;

  if (6.0 * var27 < 1.0) b = var21 + (var19 - var21) * 6.0 * var27;
  else if (2.0 * var27 < 1.0) b = var19;
  else if (3.0 * var27 < 2.0) b = var21 + (var19 - var21) * (0.666666 - var27) * 6.0;
  else b = var21;

  return vec3(pow(r, bright), pow(g, bright), pow(b, bright));
}
...
void main() {
  ...
  // This matches the original CPU-side calculation
  vec3 toCam = camPos - iCenter;
  ...
  gl_Position = worldProj * vec4(worldPos, 1.0);

  // Extract alpha (inverted: 0-255 -> 1.0-0.0)
  float alpha = 1.0 - float((iAbhsl >> 24) & 0xff) / 255.0;

  // Extract HSL and convert to RGB
  vec3 hsl = vec3((iAbhsl >> 10) & 63, (iAbhsl >> 7) & 7, iAbhsl & 127);
  vec3 rgb = hslToRgb(hsl, brightness);

  fColor = vec4(rgb, alpha);

  // Pass texture ID to fragment shader (mask to 16 bits to handle signed short)
  fTextureId = iTex.x & 0xFFFF;

  // Base UV coordinates
  vec2 baseUv = uvCoords[cornerIdx];

  // Always compute sprite UVs - for 1x1 grids this results in identity transform
  ...
  fUv = vec2(uOffset + baseUv.x * uScale, vOffset + baseUv.y * vScale);
}
