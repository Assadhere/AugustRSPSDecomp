...
class CompactBoostsOverlay extends Overlay {
   ...
   public Dimension render(Graphics2D graphics) {
      ...
      if (!this.config.compactDisplay()) {
         ...
         this.curY = this.maxX = 0;
         FontMetrics fontMetrics = graphics.getFontMetrics();
         int fontHeight = fontMetrics.getHeight();
         Iterator var5 = boostedSkills.iterator();

         while(var5.hasNext()) {
            Skill skill = (Skill)var5.next();
         ...
         time = this.plugin.getChangeDownTicks();
         if (time != -1) {
   ...
   private void drawBoost(Graphics2D graphics, FontMetrics fontMetrics, int fontHeight, BufferedImage image, Color color, String text) {
      graphics.drawImage(image, 0, this.curY, (ImageObserver)null);
