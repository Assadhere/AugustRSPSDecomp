...
public class CrowdsourcingThieving {
   ...
   private boolean hasGlovesOfSilence() {
      ...
      return equipmentContainer == null ? false : equipmentContainer.contains(10075);
   }
   ...
   public void onChatMessage(ChatMessage event) {
      if (event.getType() == ChatMessageType.SPAM) {
         ...
         if ("You smack the bandit over the head and render them unconscious.".equals(message) || "Your blow only glances off the bandit's head.".equals(message) || PICKPOCKET_FAIL.matcher(message).matches() || PICKPOCKET_SUCCESS.matcher(message).matches()) {
