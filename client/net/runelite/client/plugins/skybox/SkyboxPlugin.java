...
public class SkyboxPlugin extends Plugin {
   ...
   public void startUp() throws IOException {
      InputStream in = SkyboxPlugin.class.getResourceAsStream("skybox.txt");
   ...
   SkyboxPluginConfig provideConfig(ConfigManager configManager) {
      return (SkyboxPluginConfig)configManager.getConfig(SkyboxPluginConfig.class);
   }

   private int mapChunk(int cx, int cy, int plane) {
      cx -= this.client.getBaseX() / 8;
      cy -= this.client.getBaseY() / 8;
      int[][] instanceTemplateChunks = this.client.getInstanceTemplateChunks()[plane];
      return cx >= 0 && cx < instanceTemplateChunks.length && cy >= 0 && cy < instanceTemplateChunks[cx].length ? instanceTemplateChunks[cx][cy] : -1;
   }
   ...
   public void onBeforeRender(BeforeRender r) {
      if (this.skybox != null && this.client.getGameState() == GameState.LOGGED_IN) {
         ...
         if (player != null) {
            Color overrideColor = WorldPoint.getMirrorPoint(player.getWorldLocation(), true).getY() < 4160 ? this.config.customOverworldColor() : this.config.customOtherColor();
            if (overrideColor != null) {
               ...
               this.client.setSkyboxColor(this.skybox.getColorForPoint((double)((float)baseX + (float)(px + spx) / 128.0F), (double)((float)baseY + (float)(py + spy) / 128.0F), baseX + px / 128, baseY + py / 128, this.client.getPlane(), this.client.getTextureProvider().getBrightness(), this.client.isInInstancedRegion() ? this::mapChunk : null));
            }
         }
      }
   }
