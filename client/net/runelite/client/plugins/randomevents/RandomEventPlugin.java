...
public class RandomEventPlugin extends Plugin {
   ...
   private static final Set<Integer> EVENT_NPCS = ImmutableSet.of(6747, 5426, 307, 314, 322, 6749, new Integer[]{390, 6754, 6744, 6748, 5429, 326, 327, 5438, 5441, 6746, 5437, 5440, 6750, 6751, 6752, 6753, 5436, 5439, 380, 6738, 6755, 375, 376, 5510, 6743, 12551, 12552});
   ...
   public void onMenuEntryAdded(MenuEntryAdded event) {
      if (event.getType() >= MenuAction.NPC_FIRST_OPTION.getId() && event.getType() <= MenuAction.NPC_FIFTH_OPTION.getId() && EVENT_OPTIONS.contains(event.getOption())) {
         NPC npc = event.getMenuEntry().getNpc();
         if (npc != null && EVENT_NPCS.contains(npc.getId()) && npc != this.currentRandomEvent && this.config.removeMenuOptions()) {
   ...
   private Notification shouldNotify(int id) {
      ...
      return notification != null && notification.isEnabled() ? notification : this.config.notifyAllEvents();
   }
}
