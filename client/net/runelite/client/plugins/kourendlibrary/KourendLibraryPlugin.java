...
public class KourendLibraryPlugin extends Plugin {
   ...
   private QuestState depthsOfDespairState;
   private final Set<NPC> npcsToMark;

   public KourendLibraryPlugin() {
      this.depthsOfDespairState = QuestState.FINISHED;
      this.npcsToMark = new HashSet();
   }
   ...
   public void onConfigChanged(ConfigChanged ev) {
      if ("kourendLibrary".equals(ev.getGroup())) {
         if (ev.getKey().equals("hideVarlamoreEnvoy")) {
            KourendLibraryPanel var10000 = this.panel;
            Objects.requireNonNull(var10000);
            SwingUtilities.invokeLater(var10000::reload);
            ...
            SwingUtilities.invokeLater(() -> {
               if (!this.config.hideButton()) {
                  ...
                  boolean inRegion = lp != null && lp.getWorldLocation().getRegionID() == 6459;
                  if (inRegion) {
                     this.clientToolbar.addNavigation(this.navButton);
                  } else {
                     this.clientToolbar.removeNavigation(this.navButton);
   ...
   public void onMenuOptionClicked(MenuOptionClicked menuOpt) {
      if (MenuAction.GAME_OBJECT_FIRST_OPTION == menuOpt.getMenuAction() && menuOpt.getMenuTarget().contains("Bookshelf")) {
         this.lastBookcaseClick = WorldPoint.fromScene(this.client, menuOpt.getParam0(), menuOpt.getParam1(), this.client.getPlane());
      }
   ...
   public void onChatMessage(ChatMessage event) {
      if (event.getType() == ChatMessageType.GAMEMESSAGE) {
         if (this.lastBookcaseAnimatedOn != null && event.getMessage().equals("You don't find anything useful here.")) {
            this.library.mark((WorldPoint)this.lastBookcaseAnimatedOn, (Book)null);
   ...
   public void onGameTick(GameTick tick) {
      boolean inRegion = this.client.getLocalPlayer().getWorldLocation().getRegionID() == 6459;
      ...
      if (inRegion) {
         ...
         Widget npcHead;
         if (this.lastBookcaseAnimatedOn != null) {
            npcHead = this.client.getWidget(12648449);
            if (npcHead != null) {
               Book book = Book.byId(npcHead.getItemId());
               if (book != null) {
         ...
         npcHead = this.client.getWidget(15138818);
         if (npcHead != null && isLibraryCustomer(npcHead.getModelId())) {
            ...
            if (m.find()) {
               ...
               this.library.setCustomer(npcHead.getModelId(), book);
               this.updateBooksPanel();
               ...
               this.library.setCustomer(-1, (Book)null);
               this.updateBooksPanel();
            }
         }
   ...
   private void updateBookcaseHintArrow() {
      ...
      if (customerBook != null && !this.doesPlayerContainBook(customerBook) && this.config.showTargetHintArrow()) {
         if (state == SolvedState.COMPLETE && this.client.getHintArrowPoint() == null) {
            Iterator var3 = this.library.getBookcases().iterator();

            while(var3.hasNext()) {
               Bookcase bookcase = (Bookcase)var3.next();
               Set<Book> books = bookcase.getPossibleBooks();
               if (!books.isEmpty()) {
                  Book book = (Book)books.iterator().next();
