...
public class ExaminePlugin extends Plugin {
   ...
   public void onChatMessage(ChatMessage event) {
      if (!this.pending.isEmpty()) {
         PendingExamine pendingExamine = (PendingExamine)this.pending.peek();
   ...
   private int[] findItemFromWidget(int widgetId, int childIdx) {
      ...
      if (widget == null) {
         ...
         if (387 == widgetGroup) {
            ...
            widgetItem = widget.getChild(childIdx);
            if (widgetItem != null) {
   ...
   void getItemPrice(int id, ItemComposition itemComposition, int quantity) {
      ...
      if (gePrice > 0 || alchPrice > 0) {
         ...
         if (quantity > 1) {
            message.append(QuantityFormatter.formatNumber((long)quantity)).append(" x ");
         }
         ...
         if (gePrice > 0) {
            ...
            if (quantity > 1) {
               message.append(ChatColorType.NORMAL).append(" (").append(ChatColorType.HIGHLIGHT).append(QuantityFormatter.formatNumber((long)gePrice)).append(ChatColorType.NORMAL).append("ea)");
            }
         }
         ...
         if (alchPrice > 0) {
            ...
            if (quantity > 1) {
               message.append(ChatColorType.NORMAL).append(" (").append(ChatColorType.HIGHLIGHT).append(QuantityFormatter.formatNumber((long)alchPrice)).append(ChatColorType.NORMAL).append("ea)");
            }
         }
