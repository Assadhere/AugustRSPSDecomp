...
public class SpellbookPlugin extends Plugin {
   ...
   private static final WidgetMenuOption FIXED_MAGIC_TAB_LOCK = new WidgetMenuOption("Disable spell reordering", "", new int[]{35913798});
   private static final WidgetMenuOption FIXED_MAGIC_TAB_UNLOCK = new WidgetMenuOption("Enable spell reordering", "", new int[]{35913798});
   private static final WidgetMenuOption RESIZABLE_MAGIC_TAB_LOCK = new WidgetMenuOption("Disable spell reordering", "", new int[]{10551361});
   private static final WidgetMenuOption RESIZABLE_MAGIC_TAB_UNLOCK = new WidgetMenuOption("Enable spell reordering", "", new int[]{10551361});
   private static final WidgetMenuOption RESIZABLE_BOTTOM_LINE_MAGIC_TAB_LOCK = new WidgetMenuOption("Disable spell reordering", "", new int[]{10747962});
   private static final WidgetMenuOption RESIZABLE_BOTTOM_LINE_MAGIC_TAB_UNLOCK = new WidgetMenuOption("Enable spell reordering", "", new int[]{10747962});
   @Inject
   ...
   public void resetConfiguration() {
      Iterator var1 = this.configManager.getConfigurationKeys("spellbook.spell_").iterator();
   ...
   private void refreshReorderMenus() {
      this.clearReoderMenus();
      if (this.reordering) {
         this.menuManager.addManagedCustomMenu(FIXED_MAGIC_TAB_LOCK, (e) -> {
            this.reordering(false);
         });
         this.menuManager.addManagedCustomMenu(RESIZABLE_MAGIC_TAB_LOCK, (e) -> {
            this.reordering(false);
         });
         this.menuManager.addManagedCustomMenu(RESIZABLE_BOTTOM_LINE_MAGIC_TAB_LOCK, (e) -> {
            this.reordering(false);
         });
      } else {
         this.menuManager.addManagedCustomMenu(FIXED_MAGIC_TAB_UNLOCK, (e) -> {
            this.reordering(true);
         });
         this.menuManager.addManagedCustomMenu(RESIZABLE_MAGIC_TAB_UNLOCK, (e) -> {
            this.reordering(true);
         });
         this.menuManager.addManagedCustomMenu(RESIZABLE_BOTTOM_LINE_MAGIC_TAB_UNLOCK, (e) -> {
            this.reordering(true);
         });
      }
   ...
   public void onScriptPreFired(ScriptPreFired event) {
      if (event.getScriptId() == 2616) {
         ...
         this.clientThread.invokeLater(() -> {
            this.initializeSpells(spellBookEnum);
         });
      }
      ...
      if (this.client.getMouseCurrentButton() == 0) {
         ...
         if (draggedWidget != null && draggedOnWidget != null) {
            ...
            if (draggedGroupId == 218 && draggedOnGroupId == 218) {
               ...
               log.debug("Set {} to {}", this.client.getItemDefinition(spellbook.getIntValue(order[fromIdx])).getStringValue(601), toIdx);
               this.setPosition(spellbookId, spellbook.getIntValue(order[fromIdx]), toIdx);
               int i;
               if (fromIdx < toIdx) {
                  for(i = fromIdx + 1; i <= toIdx; ++i) {
                     log.debug("Set {} to {}", this.client.getItemDefinition(spellbook.getIntValue(order[i])).getStringValue(601), i - 1);
   ...
   public void onScriptCallbackEvent(ScriptCallbackEvent event) {
      if ("spellbookSort".equals(event.getEventName())) {
         ...
         for(int i = 0; i < numSpells; ++i) {
            ItemComposition spellObj = this.client.getItemDefinition(spellbookEnum.getIntValue(spells[i]));
            Widget w = this.client.getWidget(spellObj.getIntValue(596));
            ...
            if (this.reordering) {
               ...
               if (hidden) {
                  ...
                  newSpells[numNewSpells++] = spells[i];
                  w.setOpacity(0);
                  w.setAction(6, (String)null);
               }

               widgetConfig &= -1179649;
            }
         ...
         for(int i = 0; i < order.length; indices[order[i]] = i++) {
         }

         newSpells = Arrays.stream(newSpells, 0, numNewSpells).boxed().sorted(Comparator.comparingInt((ix) -> {
            return indices[ix];
         })).mapToInt((ix) -> {
            return ix;
         }).toArray();
   ...
   private void createWarning(boolean unlocked) {
      Widget w = this.client.getWidget(14286848);
      w.deleteAllChildren();
      if (unlocked) {
   ...
   private void redrawSpellbook() {
      ...
      if (w != null && w.getOnInvTransmitListener() != null) {
         this.client.createScriptEventBuilder(w.getOnInvTransmitListener()).setSource(w).build().run();
      }
   ...
   private int[] calculateSpellbookOrder(int spellbookId, EnumComposition spellbook) {
      ...
      for(int i = 0; i < spells.length; ++i) {
         int pos = this.getPosition(spellbookId, spellbook.getIntValue(spells[i]));
         indices[spells[i]] = pos != -1 ? pos : i;
      }

      return Arrays.stream(spells).boxed().sorted(Comparator.comparingInt((ix) -> {
         return indices[ix];
      })).mapToInt((ix) -> {
         return ix;
      }).toArray();
   ...
   private int[] defaultSpellbookOrder(EnumComposition spellbook) {
      return IntStream.range(0, spellbook.size()).boxed().sorted((idx1, idx2) -> {
         ItemComposition i1 = this.client.getItemDefinition(spellbook.getIntValue(idx1));
         ItemComposition i2 = this.client.getItemDefinition(spellbook.getIntValue(idx2));
   ...
   private boolean isHidden(int spellbook, int spell) {
      Boolean b = (Boolean)this.configManager.getConfiguration("spellbook", "spell_hidden_book_" + spellbook + "_" + spell, (Type)Boolean.TYPE);
      return b == Boolean.TRUE;
   }

   private void setHidden(int spellbook, int spell, boolean hidden) {
      if (hidden) {
         this.configManager.setConfiguration("spellbook", "spell_hidden_book_" + spellbook + "_" + spell, (Object)true);
      } else {
   ...
   private int getPosition(int spellbook, int spell) {
      Integer pos = (Integer)this.configManager.getConfiguration("spellbook", "spell_pos_book_" + spellbook + "_" + spell, (Type)Integer.TYPE);
      return pos == null ? -1 : pos;
   }

   private void setPosition(int spellbook, int spell, int position) {
      this.configManager.setConfiguration("spellbook", "spell_pos_book_" + spellbook + "_" + spell, (Object)position);
   }
}
