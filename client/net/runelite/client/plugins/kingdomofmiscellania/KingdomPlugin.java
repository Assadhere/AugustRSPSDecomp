...
public class KingdomPlugin extends Plugin {
   ...
   private void createNotification() {
      if (this.config.shouldSendNotifications() && this.isThroneOfMiscellaniaCompleted()) {
         if (this.getLastChanged() == null) {
            ...
            if (estimatedCoffer < this.config.getCofferThreshold() || getApprovalPercent(estimatedApproval) < this.config.getApprovalThreshold()) {
               this.sendChatMessage(String.format("Your Kingdom of Miscellania approval is %d%%, and your coffer has %s coins.", getApprovalPercent(estimatedApproval), QuantityFormatter.quantityToStackSize((long)estimatedCoffer)));
            }
   ...
   private int estimateCoffer(Instant lastChanged, int lastCoffer) {
      ...
      int maxDailyWithdrawal = this.isRoyalTroubleCompleted() ? 75000 : '썐';
   ...
   private int estimateApproval(Instant lastChanged, int lastApproval) {
      ...
      lastApproval -= (int)((float)daysSince * dailyPercentage * 127.0F);
   ...
   private Instant getLastChanged() {
      return (Instant)this.configManager.getRSProfileConfiguration("kingdomofmiscellania", "lastChanged", Instant.class);
   }

   private void setLastChanged(Instant lastChanged) {
      this.configManager.setRSProfileConfiguration("kingdomofmiscellania", "lastChanged", lastChanged);
   }

   int getCoffer() {
      Integer coffer = (Integer)this.configManager.getRSProfileConfiguration("kingdomofmiscellania", "coffer", Integer.TYPE);
      return coffer == null ? 0 : coffer;
   }

   private void setCoffer(int coffer) {
      this.configManager.setRSProfileConfiguration("kingdomofmiscellania", "coffer", coffer);
   }

   int getApproval() {
      Integer approval = (Integer)this.configManager.getRSProfileConfiguration("kingdomofmiscellania", "approval", Integer.TYPE);
      return approval == null ? 0 : approval;
   }

   private void setApproval(int approval) {
      this.configManager.setRSProfileConfiguration("kingdomofmiscellania", "approval", approval);
   }
}
