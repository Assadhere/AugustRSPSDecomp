...
public class ItemPricesPlugin extends Plugin {
         ...
         if (last >= 0) {
            ...
            switch (action) {
               case WIDGET_TARGET_ON_WIDGET:
                  if (menuEntry.getWidget().getId() != 9764864) {
                     break;
                  }
               case WIDGET_USE_ON_ITEM:
                  if (!this.config.showWhileAlching() || !isAlching) {
                     break;
                  }
               case CC_OP:
               case ITEM_USE:
   ...
   private String makeValueTooltip(MenuEntry menuEntry) {
      if (!this.config.showGEPrice() && !this.config.showHAValue()) {
         ...
         if (componentId != 9764864 && componentId != 983043 && componentId != 31653895 && componentId != 41287681 && componentId != 44171264 && componentId != 786444 && componentId != 41353231) {
            return null;
         } else {
            Widget w = menuEntry.getWidget();
            return w == null ? null : this.getItemStackValueText(w.getItemId(), w.getItemQuantity());
         }
      }
   }

   private String getItemStackValueText(int id, int qty) {
      id = this.itemManager.canonicalize(id);
      if (id == 995) {
         return QuantityFormatter.formatNumber((long)qty) + " gp";
      } else if (id == 13204) {
         ...
         if (itemDef.getPrice() <= 0) {
            ...
            return gePrice <= 0 && haPrice <= 0 ? null : this.stackValueText(qty, gePrice, haPrice, haProfit);
         }
      }
   }
   ...
   private String stackValueText(int qty, int gePrice, int haValue, int haProfit) {
      if (gePrice > 0) {
         ...
         if (this.config.showEA() && qty > 1) {
            this.itemStringBuilder.append(" (").append(QuantityFormatter.quantityToStackSize((long)gePrice)).append(" ea)");
         }
      }
      ...
      if (haValue > 0) {
         ...
         if (this.config.showEA() && qty > 1) {
            this.itemStringBuilder.append(" (").append(QuantityFormatter.quantityToStackSize((long)haValue)).append(" ea)");
         }
      }
