...
class BarRenderer {
   ...
   private void refreshSkills() {
      this.maxValue = (Integer)this.maxValueSupplier.get();
      this.currentValue = (Integer)this.currentValueSupplier.get();
   }

   void renderBar(StatusBarsConfig config, Graphics2D graphics, int x, int y, int width, int height) {
      int filledHeight = getBarHeight(this.maxValue, this.currentValue, height);
      Color fill = (Color)this.colorSupplier.get();
      this.refreshSkills();
   ...
   private void renderIconsAndCounters(StatusBarsConfig config, Graphics2D graphics, int x, int y, int width) {
      if (width >= 16) {
         ...
         int widthOfCounter;
         if (skillIconEnabled) {
            Image icon = (Image)this.iconSupplier.get();
            if (icon != null) {
               widthOfCounter = x + width / 2 - icon.getWidth((ImageObserver)null) / 2;
               graphics.drawImage(icon, widthOfCounter, y + 4, (ImageObserver)null);
            }
         }
         ...
         if (config.enableCounter()) {
            ...
            widthOfCounter = graphics.getFontMetrics().stringWidth(counterText);
   ...
   private void renderRestore(Graphics2D graphics, int x, int y, int width, int height) {
      int heal = (Integer)this.healSupplier.get();
      if (heal > 0) {
         int filledCurrentHeight = getBarHeight(this.maxValue, this.currentValue, height);
         int filledHealHeight = getBarHeight(this.maxValue, heal, height);
         Color color = (Color)this.healColorSupplier.get();
