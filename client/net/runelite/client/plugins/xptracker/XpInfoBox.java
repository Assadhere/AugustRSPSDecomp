...
class XpInfoBox extends JPanel {
   ...
   XpInfoBox(final XpTrackerPlugin xpTrackerPlugin, final XpTrackerConfig xpTrackerConfig, Client client, JComponent panel, final Skill skill, SkillIconManager iconManager) {
      ...
      openXpTracker.addActionListener((e) -> {
         LinkBrowser.browse(XpPanel.buildXpTrackerUrl(client.getWorldType(), client.getLocalPlayer(), skill));
      });
      ...
      resetPerHour.addActionListener((e) -> {
         xpTrackerPlugin.resetSkillPerHourState(skill);
      });
      this.pauseSkill.addActionListener((e) -> {
      ...
      popupMenu.addPopupMenuListener(new PopupMenuListener() {
         public void popupMenuWillBecomeVisible(PopupMenuEvent popupMenuEvent) {
            ...
            XpInfoBox.this.canvasItem.setText(xpTrackerPlugin.hasOverlay(skill) ? "Remove from canvas" : "Add to canvas");
         }
      ...
      this.canvasItem.addActionListener((e) -> {
         if (this.canvasItem.getText().equals("Remove from canvas")) {
      ...
      this.headerPanel.add(headerSkillIcon, "West");
      ...
      this.compactSkillIcon = getSkillIcon(iconManager, skill, 25, 16, true);
      this.compactSkillIcon.setVisible(false);
      this.progressWrapper.add(this.compactSkillIcon, "West");
      this.progressWrapper.add(this.progressBar, "Center");
      ...
      this.add(this.container, "North");
   }

   void reset() {
      this.canvasItem.setText("Add to canvas");
   ...
   void update(boolean updated, boolean paused, XpSnapshotSingle xpSnapshotSingle) {
      SwingUtilities.invokeLater(() -> {
         this.rebuildAsync(updated, paused, xpSnapshotSingle);
      });
   }
   ...
   private void rebuildAsync(boolean updated, boolean skillPaused, XpSnapshotSingle xpSnapshotSingle) {
      if (updated) {
         ...
         this.progressBar.setCenterLabel((String)this.xpTrackerConfig.progressBarLabel().getValueFunc().apply(xpSnapshotSingle));
         ...
         if (this.xpTrackerConfig.showIntermediateLevels() && xpSnapshotSingle.getEndLevel() - xpSnapshotSingle.getStartLevel() > 1) {
            ...
            for(int level = xpSnapshotSingle.getStartLevel() + 1; level <= xpSnapshotSingle.getEndLevel(); ++level) {
               double relativeStartExperience = (double)(Experience.getXpForLevel(level) - xpSnapshotSingle.getStartGoalXp());
               double relativeEndExperience = (double)(xpSnapshotSingle.getEndGoalXp() - xpSnapshotSingle.getStartGoalXp());
         ...
         if (isCombatSkill(this.skill)) {
            this.progressBar.setToolTipText(String.format("<html>%s %s</html>", tooltipLabel.getValueFunc().apply(xpSnapshotSingle), tooltipLabel == XpProgressBarLabel.PERCENTAGE ? "of goal" : "till goal lvl"));
         } else {
            this.progressBar.setToolTipText(String.format("<html>%s Actions done<br/>%s Actions/hr<br/>%s %s</html>", xpSnapshotSingle.getActionsInSession(), xpSnapshotSingle.getActionsPerHour(), tooltipLabel.getValueFunc().apply(xpSnapshotSingle), tooltipLabel == XpProgressBarLabel.PERCENTAGE ? "of goal" : "till goal lvl"));
         }
   ...
   private String htmlLabel(XpPanelLabel panelLabel, XpSnapshotSingle xpSnapshotSingle) {
      if (isActions(panelLabel) && isCombatSkill(this.skill)) {
         return "";
      } else {
         String key = panelLabel.getKey() + ": ";
         String value = (String)panelLabel.getValueFunc().apply(xpSnapshotSingle);
