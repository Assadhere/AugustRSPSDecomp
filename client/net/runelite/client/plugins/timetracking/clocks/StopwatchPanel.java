...
class StopwatchPanel extends ClockPanel {
   private static final Color LAP_DATA_COLOR;
   ...
   private void rebuildLapList() {
      ...
      if (laps.isEmpty()) {
         ...
         for(Iterator var5 = this.stopwatch.getLaps().iterator(); var5.hasNext(); ++c.gridy) {
            long lap = (Long)var5.next();
            c.gridx = 0;
            this.lapsContainer.add(this.createSmallLabel("" + (c.gridy + 1)), c);
            c.gridx = 1;
            this.lapsContainer.add(this.createSmallLabel(getFormattedDuration(lap - previousLap)), c);
            c.gridx = 2;
            this.lapsContainer.add(this.createSmallLabel(getFormattedDuration(lap)), c);
            previousLap = lap;
         }
      }
   ...
   static {
      LAP_DATA_COLOR = ColorScheme.LIGHT_GRAY_COLOR.darker();
   }
}
