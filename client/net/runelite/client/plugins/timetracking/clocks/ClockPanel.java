...
abstract class ClockPanel extends JPanel {
   ...
   private static final Color ACTIVE_CLOCK_COLOR;
   private static final Color INACTIVE_CLOCK_COLOR;
   ...
   ClockPanel(final ClockManager clockManager, final Clock clock, String clockType, boolean editable) {
      ...
      this.nameInput.addActionListener((e) -> {
      ...
      this.displayInput.setBorder((Border)null);
      ...
      this.displayInput.addActionListener((e) -> {
         this.getParent().requestFocusInWindow();
      });
      ...
      actionsBar.add(this.leftActions, "West");
      actionsBar.add(this.rightActions, "East");
      mainContainer.add(this.contentContainer, "Center");
      mainContainer.add(actionsBar, "South");
      this.add(nameWrapper, "North");
      this.add(mainContainer, "Center");
   }
   ...
   static long stringToSeconds(String time) throws NumberFormatException, DateTimeParseException {
      long duration = 0L;
      if (time.matches(".*[hms].*")) {
         String textWithoutWhitespaces = time.replaceAll("\\s+", "");
         duration = Duration.parse("PT" + textWithoutWhitespaces).toMillis() / 1000L;
      } else {
         String[] parts = time.split(":");
         int i = parts.length - 1;

         for(int multiplier = 1; i >= 0 && multiplier <= 3600; multiplier *= 60) {
   ...
   static {
      ...
      ACTIVE_CLOCK_COLOR = ColorScheme.LIGHT_GRAY_COLOR.brighter();
      INACTIVE_CLOCK_COLOR = ColorScheme.LIGHT_GRAY_COLOR.darker();
   }
}
