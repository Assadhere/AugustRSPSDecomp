...
public class FarmingContractManager {
   ...
   public boolean shouldHighlightFarmingTabPanel(@Nonnull FarmingPatch patch) {
      ...
      return this.contract != null && patch.getRegion().getRegionID() == 4922 && this.contract.getPatchImplementation() == patch.getImplementation() && patchPrediction != null && (this.summary == SummaryState.EMPTY && (patchPrediction.getProduce() == null || patchPrediction.getProduce() == Produce.WEEDS) || patchPrediction.getProduce().equals(this.contract));
   }
   ...
   private void handleGuildmasterJaneWidgetDialog() {
      ...
      if (npcDialog != null && npcDialog.getModelId() == 8628) {
         ...
         if (dialogText.equals("You'll be wanting a reward then. Here you go.")) {
            this.setContract((Produce)null);
         }
   ...
   private void handleContractState() {
      if (this.contract == null) {
         ...
         FarmingPatch[] var5 = this.farmingWorld.getFarmingGuildRegion().getPatches();
            ...
            if (patch.getImplementation() == patchImplementation) {
               ...
               if (prediction != null) {
                  ...
                  if (produce != this.contract && produce != Produce.ANYHERB) {
                     if (!hasEmptyPatch && !hasDiseasedPatch && !hasDeadPatch && this.completionTime == Long.MAX_VALUE) {
                        this.summary = SummaryState.OCCUPIED;
                     }
                  } else if ((state != CropState.DEAD || !hasDiseasedPatch && this.completionTime == Long.MAX_VALUE) && (state != CropState.DISEASED || this.completionTime == Long.MAX_VALUE)) {
   ...
   private Produce getStoredContract() {
      try {
         return Produce.getByItemID(Integer.parseInt(this.configManager.getRSProfileConfiguration("timetracking", "contract")));
   ...
   private void setStoredContract(@Nullable Produce contract) {
      if (contract != null) {
         this.configManager.setRSProfileConfiguration("timetracking", "contract", String.valueOf(contract.getItemID()));
      } else {
         this.configManager.unsetRSProfileConfiguration("timetracking", "contract");
      }
