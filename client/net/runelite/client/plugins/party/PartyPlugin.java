...
public class PartyPlugin extends Plugin {
   ...
   protected void shutDown() throws Exception {
      ...
      WorldMapPointManager var10000 = this.worldMapManager;
      Objects.requireNonNull(PartyWorldMapPoint.class);
      var10000.removeIf(PartyWorldMapPoint.class::isInstance);
   ...
   void leaveParty() {
      this.party.changeParty((String)null);
   }
   ...
   public void onConfigChanged(ConfigChanged event) {
      if (event.getGroup().equals("party")) {
         ...
         PartyPanel var10000 = this.panel;
         Objects.requireNonNull(var10000);
         SwingUtilities.invokeLater(var10000::updateAll);
      }
   ...
   public void onMenuOptionClicked(MenuOptionClicked event) {
      if (this.hotkeyPressed && !this.client.isMenuOpen() && this.party.isInParty() && this.config.pings()) {
         ...
         if (selectedSceneTile != null) {
            ...
            MenuEntry[] var4 = this.client.getMenuEntries();
   ...
   public void onStatusUpdate(StatusUpdate event) {
      ...
      if (partyData != null) {
         ...
         SwingUtilities.invokeLater(() -> {
            this.panel.updateMember(event.getMemberId());
         });
      }
   }
   ...
   private void checkStateChanged(boolean forceSend) {
      ...
      if (this.party.isInParty()) {
         ...
         if (!canDelay || this.client.getTickCount() % messageFreq(this.party.getMembers().size()) == 0) {
            if (hasChange) {
   ...
   public void onUserPart(UserPart event) {
      PartyData removed = (PartyData)this.partyDataMap.remove(event.getMemberId());
      if (removed != null) {
         this.worldMapManager.remove(removed.getWorldMapPoint());
         SwingUtilities.invokeLater(() -> {
            this.panel.removeMember(event.getMemberId());
         });
      }
   ...
   public void onPartyChanged(PartyChanged event) {
      ...
      PartyPanel var2 = this.panel;
      Objects.requireNonNull(var2);
      SwingUtilities.invokeLater(var2::removeAllMembers);
   }
   ...
   public void onCommandExecuted(CommandExecuted commandExecuted) {
      if (this.developerMode && commandExecuted.getCommand().equalsIgnoreCase("partyinfo")) {
         ...
         QueuedMessage.QueuedMessageBuilder var10001 = QueuedMessage.builder().type(ChatMessageType.GAMEMESSAGE);
         String var10002 = this.party.getPartyPassphrase();
         var10000.queue(var10001.value("Party " + var10002 + " ID " + this.party.getPartyId()).build());
         ...
         while(var2.hasNext()) {
            PartyMember partyMember = (PartyMember)var2.next();
            var10000 = this.chatMessageManager;
            var10001 = QueuedMessage.builder().type(ChatMessageType.GAMEMESSAGE);
            var10002 = partyMember.getDisplayName();
            var10000.queue(var10001.value("Member " + var10002 + " " + partyMember.getMemberId()).build());
         }
   ...
   PartyData getPartyData(long uuid) {
      ...
      return memberById == null ? null : (PartyData)this.partyDataMap.computeIfAbsent(uuid, (u) -> {
         WorldMapPoint worldMapPoint = new PartyWorldMapPoint(new WorldPoint(0, 0, 0), memberById);
