...
class PartyPanel extends PluginPanel {
   ...
   PartyPanel(ClientThread clientThread, PartyPlugin plugin, PartyConfig config, PartyService party) {
      ...
      c.gridx = 0;
      c.gridy = 0;
      topPanel.add(this.startButton, c);
      c.gridx = 1;
      c.gridy = 0;
      topPanel.add(this.joinPartyButton, c);
      c.gridx = 1;
      c.gridy = 0;
      topPanel.add(this.copyPartyIdButton, c);
      c.gridx = 0;
      c.gridy = 1;
      c.gridwidth = 2;
      topPanel.add(this.rejoinPartyButton, c);
      layoutPanel.add(topPanel);
      layoutPanel.add(this.memberBoxPanel);
      this.startButton.setText(party.isInParty() ? "Leave" : "Create party");
      ...
      this.startButton.addActionListener((e) -> {
         if (party.isInParty()) {
            int result = JOptionPane.showOptionDialog(this.startButton, "Are you sure you want to leave the party?", "Leave party?", 0, 2, (Icon)null, new String[]{"Yes", "No"}, "No");
            if (result == 0) {
               plugin.leaveParty();
            }
         } else {
            clientThread.invokeLater(() -> {
               party.changeParty(party.generatePassphrase());
            });
         }
      ...
      this.joinPartyButton.addActionListener((e) -> {
         if (!party.isInParty()) {
            String s = (String)JOptionPane.showInputDialog(this.joinPartyButton, "Please enter the party passphrase:", "Party Passphrase", -1, (Icon)null, (Object[])null, "");
            if (s == null) {
               return;
            }
      ...
      this.copyPartyIdButton.addActionListener((e) -> {
         if (party.isInParty()) {
            ...
            clipboard.setContents(new StringSelection(party.getPartyPassphrase()), (ClipboardOwner)null);
         }
   ...
   void updateParty() {
      ...
      this.startButton.setText(this.party.isInParty() ? "Leave" : "Create party");
   ...
   void removeAllMembers() {
      this.memberBoxes.forEach((key, value) -> {
         this.memberBoxPanel.remove(value);
      });
      this.memberBoxPanel.revalidate();
   ...
   void removeMember(long memberId) {
      PartyMemberBox memberBox = (PartyMemberBox)this.memberBoxes.remove(memberId);
      if (memberBox != null) {
   ...
   void updateMember(long userId) {
      PartyMemberBox memberBox = (PartyMemberBox)this.memberBoxes.get(userId);
      if (memberBox != null) {
         memberBox.update();
      }
