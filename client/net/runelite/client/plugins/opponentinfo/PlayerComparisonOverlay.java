...
class PlayerComparisonOverlay extends Overlay {
   private static final Color HIGHER_STAT_TEXT_COLOR;
   private static final Color LOWER_STAT_TEXT_COLOR;
   ...
   private void generateComparisonTable(PanelComponent panelComponent, HiscoreResult opponentSkills) {
      ...
      panelComponent.getChildren().add(LineComponent.builder().left("Skill").leftColor(HIGHLIGHT_COLOR).right("You/Them").rightColor(HIGHLIGHT_COLOR).build());
      ...
      for(int i = 0; i < COMBAT_SKILLS.length; ++i) {
         ...
         if (opponentSkill != null && opponentSkill.getLevel() != -1) {
            ...
            List var10000 = panelComponent.getChildren();
            LineComponent.LineComponentBuilder var10001 = LineComponent.builder().left(hiscoreSkill.getName());
            String var10002 = Integer.toString(playerSkillLevel);
            var10000.add(var10001.right(var10002 + "/" + Integer.toString(opponentSkillLevel)).rightColor(comparisonStatColor(playerSkillLevel, opponentSkillLevel)).build());
         }
      }
   ...
   private static Color comparisonStatColor(int a, int b) {
      if (a > b) {
         ...
         return a < b ? LOWER_STAT_TEXT_COLOR : NEUTRAL_TEXT_COLOR;
      }
   }

   static {
      HIGHER_STAT_TEXT_COLOR = Color.GREEN;
      LOWER_STAT_TEXT_COLOR = Color.RED;
      NEUTRAL_TEXT_COLOR = Color.WHITE;
      HIGHLIGHT_COLOR = new Color(255, 200, 0, 255);
      COMBAT_SKILLS = new Skill[]{Skill.ATTACK, Skill.STRENGTH, Skill.DEFENCE, Skill.HITPOINTS, Skill.RANGED, Skill.MAGIC, Skill.PRAYER};
      HISCORE_COMBAT_SKILLS = new HiscoreSkill[]{HiscoreSkill.ATTACK, HiscoreSkill.STRENGTH, HiscoreSkill.DEFENCE, HiscoreSkill.HITPOINTS, HiscoreSkill.RANGED, HiscoreSkill.MAGIC, HiscoreSkill.PRAYER};
   }
}
