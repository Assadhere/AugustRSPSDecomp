...
public class WintertodtPlugin extends Plugin {
   ...
   private WintertodtActivity currentActivity;
   ...
   private void checkActionTimeout() {
      if (this.currentActivity != WintertodtActivity.IDLE) {
         int currentAnimation = this.client.getLocalPlayer() != null ? this.client.getLocalPlayer().getAnimation() : -1;
         ...
         if (chatMessageType == ChatMessageType.GAMEMESSAGE || chatMessageType == ChatMessageType.SPAM) {
            ...
            if (messageNode.getValue().startsWith("You carefully fletch the root")) {
               ...
               if (!neverNotify) {
                  ...
                  WintertodtNotifyDamage notify;
                  switch (interruptType) {
                     case COLD:
                        notify = this.config.notifyCold();
                        shouldNotify = notify == WintertodtNotifyDamage.ALWAYS || notify == WintertodtNotifyDamage.INTERRUPT && wasInterrupted;
                        break;
                     case BRAZIER:
                        notify = this.config.notifyBrazierDamage();
                        shouldNotify = notify == WintertodtNotifyDamage.ALWAYS || notify == WintertodtNotifyDamage.INTERRUPT && wasInterrupted;
                        break;
                     case SNOWFALL:
                        notify = this.config.notifySnowfall();
                        shouldNotify = notify == WintertodtNotifyDamage.ALWAYS || notify == WintertodtNotifyDamage.INTERRUPT && wasInterrupted;
                        break;
                     case INVENTORY_FULL:
   ...
   public void onItemContainerChanged(ItemContainerChanged event) {
      ...
      if (this.isInWintertodt && container == this.client.getItemContainer(93)) {
         ...
         for(int var6 = 0; var6 < var5; ++var6) {
            Item item = var4[var6];
            this.inventoryScore += getPoints(item.getId());
            this.totalPotentialinventoryScore += getPotentialPoints(item.getId());
            switch (item.getId()) {
               case 20695:
                  ++this.numLogs;
                  break;
