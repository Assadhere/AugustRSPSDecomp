...
public class WidgetField<T> {
   ...
   WidgetField(String name, Function<Widget, T> getter) {
      this(name, getter, (BiConsumer)null, (Class)null);
   }
   ...
   Object getValue(Widget widget) {
      Object value = this.getter.apply(widget);
      if (!(value instanceof Boolean) && !(value instanceof Number) && !(value instanceof String)) {
         return value instanceof Widget ? WidgetInspector.getWidgetIdentifier((Widget)value) : MessageFormatter.format("{}", value).getMessage();
      } else {
         return value;
      }
   }
   ...
   void setValue(Widget widget, Object inValue) {
      ...
      this.setter.accept(widget, value);
   }
