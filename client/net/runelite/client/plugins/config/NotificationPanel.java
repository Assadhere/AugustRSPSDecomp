...
class NotificationPanel extends PluginPanel {
   ...
   private ColorJButton createColorPicker(final String name, Color existing, final Consumer<Color> onClose) {
      ...
      colorPickerBtn.addMouseListener(new MouseAdapter() {
         public void mouseClicked(MouseEvent e) {
            RuneliteColorPicker colorPicker = NotificationPanel.this.colorPickerManager.create((Component)NotificationPanel.this, colorPickerBtn.getColor(), name, false);
   ...
   private void rebuild(Notification notif) {
      ...
      if (notif.isOverride()) {
         ...
         comboboxRequestFocus.addItemListener((e) -> {
            if (e.getStateChange() == 1) {
               Notification n = this.loadNotification();
               this.saveNotification(n.withRequestFocus((RequestFocusType)comboboxRequestFocus.getSelectedItem()));
            }

         });
         this.item("Request focus", "Configures the window focus request type on notification.", comboboxRequestFocus);
         ArrayList<NotificationOption> options = (ArrayList)Arrays.stream(NotificationSound.values()).map((ns) -> {
            return new NotificationOption(ns, (String)null, ns.toString());
         }).collect(Collectors.toCollection(ArrayList::new));
         options.addAll(loadCustomNotifications());
         NotificationOption existing = (NotificationOption)options.stream().filter((no) -> {
            return no.type == notif.getSound() && Objects.equals(no.soundName, notif.getSoundName());
         }).findAny().orElse((Object)null);
         if (existing == null) {
            String optionName = notif.getSoundName().substring(0, notif.getSoundName().length() - ".wav".length());
            options.add(existing = new NotificationOption(notif.getSound(), notif.getSoundName(), "<html><font color=red>" + optionName + "</font></html>"));
         }

         JComboBox<NotificationOption> comboboxSound = this.combobox((NotificationOption[])options.toArray(new NotificationOption[0]), existing);
         ...
         comboboxFlash.addItemListener((e) -> {
            if (e.getStateChange() == 1) {
               Notification n = this.loadNotification();
               this.saveNotification(n.withFlash((FlashNotification)comboboxFlash.getSelectedItem()));
            }
         ...
         ColorJButton colorpickerFlashColor = this.createColorPicker("Flash color", notif.getFlashColor(), (c) -> {
            Notification n = this.loadNotification();
            this.saveNotification(n.withFlashColor(c));
         });
         ...
         resetButton.addActionListener((e) -> {
            int result = JOptionPane.showOptionDialog(resetButton, "Are you sure you want to reset this notification configuration?", "Are you sure?", 0, 2, (Icon)null, new String[]{"Yes", "No"}, "No");
            if (result == 0) {
   ...
   private Notification loadNotification() {
      return (Notification)this.configManager.getConfiguration(this.configDescriptor.getGroup().value(), this.configItemDescriptor.getItem().keyName(), (Type)Notification.class);
   }

   private void saveNotification(Notification notification) {
      this.configManager.setConfiguration(this.configDescriptor.getGroup().value(), this.configItemDescriptor.getItem().keyName(), (Object)notification);
   }

   private static List<NotificationOption> loadCustomNotifications() {
      File[] files = RuneLite.NOTIFICATIONS_DIR.listFiles();
      return files == null ? Collections.emptyList() : (List)Arrays.stream(files).filter((f) -> {
         return f.getName().endsWith(".wav");
      }).map((f) -> {
         return new NotificationOption(NotificationSound.CUSTOM, f.getName(), f.getName().substring(0, f.getName().length() - ".wav".length()));
