...
enum GameCounter {
   ...
   CURSE_OF_THE_MOONS_BLUE(29019, GameTimerImageType.ITEM, "Curse of the Moons (Blue Moon)", GameCounter.ColorBoundaryType.GREATER_THAN_EQUAL_TO, 18, Color.RED),
   ...
   private GameCounter(int imageId, GameTimerImageType idType, String description, boolean shouldDisplayCount) {
      this(imageId, idType, description, GameCounter.ColorBoundaryType.NO_BOUNDARY, 0, Color.WHITE, shouldDisplayCount);
   }

   private GameCounter(int imageId, GameTimerImageType idType, String description) {
      this(imageId, idType, description, GameCounter.ColorBoundaryType.NO_BOUNDARY, 0, Color.WHITE, true);
   }
   ...
   static enum ColorBoundaryType {
      GREATER_THAN_EQUAL_TO((count, boundary) -> {
         return count >= boundary;
      }),
      LESS_THAN_EQUAL_TO((count, boundary) -> {
         return count <= boundary;
      }),
      NO_BOUNDARY((count, boundary) -> {
         return false;
      });
      ...
      private ColorBoundaryType(BiPredicate shouldRecolorPredicate) {
