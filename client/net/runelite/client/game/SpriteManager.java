...
public class SpriteManager {
   ...
   private final Cache<Long, BufferedImage> cache;

   @Inject
   private SpriteManager(Client client, ClientThread clientThread, InfoBoxManager infoBoxManager) {
      this.cache = CacheBuilder.newBuilder().maximumSize(128L).expireAfterAccess(1L, TimeUnit.HOURS).build();
      this.client = client;
   ...
   public BufferedImage getSprite(int archive, int file) {
      ...
      if (this.client.getGameState().ordinal() < GameState.LOGIN_SCREEN.ordinal()) {
         ...
         BufferedImage cached = (BufferedImage)this.cache.getIfPresent(key);
         if (cached != null) {
            return cached;
         } else {
   ...
   public void getSpriteAsync(int archive, int file, Consumer<BufferedImage> user) {
      BufferedImage cached = (BufferedImage)this.cache.getIfPresent((long)archive << 32 | (long)file);
      if (cached != null) {
         user.accept(cached);
      } else {
   ...
   public void getSpriteAsync(int archive, int file, InfoBox infoBox) {
      this.getSpriteAsync(archive, file, (img) -> {
   ...
   public void addSpriteOverrides(SpriteOverride[] add) {
      if (add.length > 0) {
         this.clientThread.invokeLater(() -> {
            ...
            SpriteOverride[] var4 = add;
            int var5 = add.length;

            for(int var6 = 0; var6 < var5; ++var6) {
               SpriteOverride o = var4[var6];
   ...
   public void removeSpriteOverrides(SpriteOverride[] remove) {
      this.clientThread.invokeLater(() -> {
         Map<Integer, SpritePixels> overrides = this.client.getSpriteOverrides();
         SpriteOverride[] var3 = remove;
         int var4 = remove.length;

         for(int var5 = 0; var5 < var4; ++var5) {
            SpriteOverride o = var3[var5];
