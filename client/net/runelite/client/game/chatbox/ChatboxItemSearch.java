...
public class ChatboxItemSearch extends ChatboxTextInput {
   ...
   protected void update() {
      ...
      separator.setTextColor(6710886);
      separator.revalidate();
      int x = 6;
      int y = 18;
      int idx = 0;

      for(Iterator var7 = this.results.values().iterator(); var7.hasNext(); ++idx) {
         ItemComposition itemComposition = (ItemComposition)var7.next();
   ...
   private void filterResults() {
      ...
      if (!search.isEmpty()) {
         ...
         for(int i = 0; i < this.client.getItemCount() && this.results.size() < 24; ++i) {
            ...
            if (!name.equals("null") && name.contains(search) && !this.results.containsKey(itemComposition.getId())) {
               ItemIcon itemIcon = new ItemIcon(itemComposition.getInventoryModel(), itemComposition.getAmbient(), itemComposition.getContrast(), itemComposition.getColorToReplaceWith(), itemComposition.getTextureToReplaceWith());
   ...
   private static final class ItemIcon {
      ...
      public boolean equals(Object o) {
         if (o == this) {
            return true;
         } else if (!(o instanceof ItemIcon)) {
            return false;
         } else {
            ItemIcon other = (ItemIcon)o;
            if (this.getModelId() != other.getModelId()) {
               return false;
            } else if (this.getAmbient() != other.getAmbient()) {
               return false;
            } else if (this.getContrast() != other.getContrast()) {
               return false;
            } else if (!Arrays.equals(this.getColorsToReplace(), other.getColorsToReplace())) {
               return false;
            } else {
      ...
      public String toString() {
         int var10000 = this.getModelId();
         return "ChatboxItemSearch.ItemIcon(modelId=" + var10000 + ", ambient=" + this.getAmbient() + ", contrast=" + this.getContrast() + ", colorsToReplace=" + Arrays.toString(this.getColorsToReplace()) + ", texturesToReplace=" + Arrays.toString(this.getTexturesToReplace()) + ")";
      }
   }
}
