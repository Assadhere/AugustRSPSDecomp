...
public class QuantityFormatter {
   ...
   public static synchronized String quantityToStackSize(long quantity) {
      if (quantity < 0L) {
         ...
         for(int i = SUFFIXES.length - 1; i >= 0; --i) {
            divideBy = (long)Math.pow(10.0, (double)(i * 3));
            if ((double)quantity / (double)divideBy >= 1.0) {
               suffix = SUFFIXES[i];
               break;
            }
         }
   ...
   public static synchronized String quantityToRSDecimalStack(int quantity, boolean precise) {
      ...
      if (quantityStr.length() <= 4) {
         ...
         String var10000 = format.format((double)quantity / Math.pow(10.0, (double)(power / 3 * 3)));
         ...
         if (!suffix.equals("")) {
            for(int i = 1; i < SUFFIXES.length; ++i) {
               if (SUFFIXES[i].equals(suffix.toUpperCase())) {
                  return (int)Math.pow(10.0, (double)(i * 3));
               }
            }
