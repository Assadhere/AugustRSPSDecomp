...
public class FatalErrorDialog extends JDialog {
   ...
   public FatalErrorDialog addHelpButtons() {
      return this.addButton("Open logs folder", () -> {
   ...
   public FatalErrorDialog addBuildingGuide() {
      return this.addButton("Building guide", () -> {
         LinkBrowser.browse(RuneLiteProperties.getBuildingLink());
      });
   }
   ...
   public static void showNetErrorWindow(String action, Throwable err) {
      if (!(err instanceof VerificationException) && !(err instanceof GeneralSecurityException)) {
         if (err instanceof ConnectException) {
            ...
            (new FatalErrorDialog("RuneLite is unable to resolve the address of a required server while " + action + ". Your DNS resolver may be misconfigured, pointing to an inaccurate resolver, or your internet connection may be down. ")).addHelpButtons().addButton("Change your DNS resolver", () -> {
               LinkBrowser.browse(RuneLiteProperties.getDNSChangeLink());
            }).open();
         } else {
