...
public class WorldMapOverlay extends Overlay {
         ...
         if (widget != null && bottomBar != null) {
            bottomBar.setOnTimerListener(new Object[]{(ev) -> {
               ...
               if (!this.client.isMenuOpen() && worldPoint != null) {
                  this.client.createMenuEntry(-1).setTarget(ColorUtil.wrapWithColorTag(worldPoint.getName(), JagexColors.MENU_TARGET)).setOption("Focus on").setType(MenuAction.RUNELITE).onClick((m) -> {
                     this.client.getWorldMap().setWorldMapPositionTarget((WorldPoint)MoreObjects.firstNonNull(worldPoint.getTarget(), worldPoint.getWorldPoint()));
                  });
               }
            }});
            ...
            if (!mapViewArea.contains((double)mousePos.getX(), (double)mousePos.getY())) {
               mousePos = null;
            }

            this.hoveredPoint = null;
            WorldMapPoint tooltipPoint = null;
            Iterator var12 = points.iterator();

            while(true) {
               WorldMapPoint worldPoint;
               BufferedImage image;
               Point drawPoint;
               do {
                  WorldPoint point;
                  do {
                     do {
                        ...
                        worldPoint = (WorldMapPoint)var12.next();
                        image = worldPoint.getImage();
                        point = worldPoint.getWorldPoint();
                     } while(image == null);
                  } while(point == null);

                  drawPoint = this.mapWorldPointToGraphicsPoint(point);
               ...
               graphics.drawImage(image, drawX, drawY, (ImageObserver)null);
   ...
   public Point mapWorldPointToGraphicsPoint(WorldPoint worldPoint) {
      ...
      if (!worldMap.getWorldMapData().surfaceContainsPosition(worldPoint.getX(), worldPoint.getY())) {
         ...
         if (map != null) {
            ...
            yGraphDiff = (int)((double)yGraphDiff - ((double)pixelsPerTile - Math.ceil((double)(pixelsPerTile / 2.0F))));
            xGraphDiff = (int)((double)xGraphDiff + ((double)pixelsPerTile - Math.ceil((double)(pixelsPerTile / 2.0F))));
   ...
   private Shape getWorldMapClipArea(Rectangle baseRectangle) {
      ...
      return (Shape)(subtracted ? clipArea : baseRectangle);
   }
   ...
   private void drawTooltip(Graphics2D graphics, WorldMapPoint worldPoint) {
      ...
      if (tooltip != null && tooltip.length() > 0 && drawPoint != null) {
         ...
         if (!rows.isEmpty()) {
            ...
            Stream var10000 = rows.stream();
            Objects.requireNonNull(fm);
            int width = (Integer)var10000.map(fm::stringWidth).max(Integer::compareTo).get();
