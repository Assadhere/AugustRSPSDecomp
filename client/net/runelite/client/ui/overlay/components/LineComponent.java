...
public class LineComponent implements LayoutableRenderableEntity {
   ...
   public Dimension render(Graphics2D graphics) {
      String left = (String)MoreObjects.firstNonNull(this.left, "");
      String right = (String)MoreObjects.firstNonNull(this.right, "");
      Font leftFont = (Font)MoreObjects.firstNonNull(this.leftFont, graphics.getFont());
      Font rightFont = (Font)MoreObjects.firstNonNull(this.rightFont, graphics.getFont());
      ...
      int x = baseX;
      int y = baseY;
      int leftFullWidth = getLineWidth(left, lfm);
      int rightFullWidth = getLineWidth(right, rfm);
      ...
      if (this.preferredSize.width < leftFullWidth + rightFullWidth) {
         ...
         String[] leftSplitLines = lineBreakText(left, leftSmallWidth, lfm);
         String[] rightSplitLines = lineBreakText(right, rightSmallWidth, rfm);
         int lineCount = Math.max(leftSplitLines.length, rightSplitLines.length);

         for(int i = 0; i < lineCount; ++i) {
            String rightText;
            if (i < leftSplitLines.length) {
               rightText = leftSplitLines[i];
            ...
            if (i < rightSplitLines.length) {
               ...
               textComponent.setPosition(x + this.preferredSize.width - getLineWidth(rightText, rfm), y);
               textComponent.setText(rightText);
         ...
         if (!right.isEmpty()) {
            textComponent.setPosition(x + this.preferredSize.width - rightFullWidth, y);
