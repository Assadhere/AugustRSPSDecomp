...
public class TooltipComponent implements LayoutableRenderableEntity {
   ...
   public Dimension render(Graphics2D graphics) {
      ...
      int tooltipWidth = 0;
      int tooltipHeight = 0;
      String[] lines = BR.split(this.text);
      String[] var8 = lines;
      int y = lines.length;
      ...
      for(int var10 = 0; var10 < y; ++var10) {
         ...
         textX = calculateTextWidth(metrics, line);
         if (textX > tooltipWidth) {
            tooltipWidth = textX;
         }
      ...
      for(int i = 0; i < lines.length; ++i) {
         ...
         for(int j = 0; j < chars.length; ++j) {
            ...
            if (chars[j] == '<') {
               ...
               argument = line.substring(begin, j);
               textComponent.setText(argument);
               textComponent.setPosition(lineX, textY + (i + 1) * textHeight - textDescent);
               textComponent.render(graphics);
               lineX += metrics.stringWidth(argument);
               begin = j;
               inTag = true;
   ...
   private void renderModIcon(Graphics2D graphics, int x, int y, IndexedSprite modIcon) {
      ...
      for(int y2 = 0; y2 < modIcon.getHeight(); ++y2) {
         for(int x2 = 0; x2 < modIcon.getWidth(); ++x2) {
            int index = modIcon.getPixels()[sourceOffset++] & 255;
            if (index != 0) {
